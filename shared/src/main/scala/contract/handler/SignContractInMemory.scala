...
trait SignContractHandler extends EnactContract {
  ...
  def run(request: SignContract): Task[SignContractResponse] =
    EnactContractLogic(request).foldMap[Task]
...
object SignContractHandler {
  ...
  class InMemory() extends SignContractHandler {
    ...
    private def sign(isCounterpartyA: Boolean, ref: CounterpartyRef): CounterpartyRef |
      NotSignedReason = {
      if isCounterpartyA then sentContractsA = ref +: sentContractsA
      else sentContractsB = ref +: sentContractsB
