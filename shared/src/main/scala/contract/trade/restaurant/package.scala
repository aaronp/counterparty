...
package object restaurant {
  ...
  opaque type Dish = List[Ingredient]
  object Dish:
    def apply(value: List[Ingredient]): Dish = value
  extension (dish: Dish)
    def dishIngredients: List[Ingredient] = dish
  ...
  opaque type Order = List[Dish]
  object Order:
    def apply(value: List[Dish]): Order = value
  extension (order: Order)
    def orderIngredients: List[Ingredient] = order.flatten
    def asInventory: Inventory             = Inventory(orderIngredients)
    def dishes: List[Dish]                 = order
