...
import contract.CreateDraftLogic.{LogMessage, NotifyCounterpartyA, NotifyCounterpartyB, StoreDraftInDatabase}
...
object CreateDraftAsMermaid {
  ...
  def apply(draft: DraftContract = DraftContract.testData): String = {
    ...
    val participants = Seq(Participants.ContractService, Participants.Database, Participants.CounterpartyA, Participants.CounterpartyB)
