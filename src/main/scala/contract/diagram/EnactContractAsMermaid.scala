...
object EnactContractAsMermaid extends EnactContract {
  ...
  given~>[EnactContractLogic, CallsState] with
    def apply[A](op: EnactContractLogic[A]): State[Calls, A] = op match {
      case RequestSignatureA(ref) =>
        s"$ContractService->>+$CounterpartyA: request signature for $ref".asState(CounterpartyA.toString)
      case RequestSignatureB(ref) =>
        s"$ContractService->>+$CounterpartyB: request signature for $ref".asState(CounterpartyB.toString)
      case Wait(counterpartyName) =>
        s"$counterpartyName->>-$ContractService: send response".asState(counterpartyName.asInstanceOf[A])
      case LogMessage(msg) => s"$ContractService->>$ContractService : log ${msg.take(18)}...".asState(())
    }

  /** function which turns the request into a mermaid sequence diagram.
   * see https://mermaid.js.org/syntax/sequenceDiagram.html
   * @param sign the request
   * @return a mermaid markup diagram
   */
  def apply(sign : SignContract = testData): String = {
    ...
    val participants = Seq(Participants.ContractService, Participants.CounterpartyA, Participants.CounterpartyB)
