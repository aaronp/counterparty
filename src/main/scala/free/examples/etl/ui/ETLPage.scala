...
case class ETLPage(containerId: String) {
  ...
  val testButton = button(`style` := "width:100%;height:90px;font-size:20pt")("send test message").render

  def lookupUser(userId : UserId): Option[String] = {
  ...
  def makeInterpreter(config : Config) = {
    ...
    stateTable.rowsForCells().foreach {
      case Row(Seq(key, value)) => stateDB.update(key, parseTime(value))
    }
  ...
  val page = div(div(`class` := "flex-container")(
    msgPage.render,
    cfgPage.render),
