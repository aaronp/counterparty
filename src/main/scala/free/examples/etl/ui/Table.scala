...
import free.examples.etl.UpstreamMessage
...
case class Table(tableId: String, initialRows: IndexedSeq[Row] = Vector()) {
   ...
   * @param byIndex the index of the field to replace if this row is the same as another
...
case class TableBody(parent: Table, initialRows: IndexedSeq[Row]) {
  ...
  private var rowsByIndex: IndexedSeq[(Int, Seq[Input])] = initialRows.zipWithIndex.map { (row, idx) =>
  ...
  val bodyRows = rowsByIndex.map { (rowIndex, cells) =>
    ...
    tr(td(`style` := "width:10px;align:center")(deleteMe).render +: cells.map(c => td(c).render)).render
  }
}
